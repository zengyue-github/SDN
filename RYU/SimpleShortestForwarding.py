...
class ExampleShortestForwarding(app_manager.RyuApp):
    ...
    def get_topology(self, ev):
        ...
        for link in links_list:
            self.network.add_edge(link.src.dpid,link.dst.dpid,weight=100)
            self.network.add_edge(link.dst.dpid,link.src.dpid,weight=100)
    ...
    def get_out_port(self, src, dst, datapath, in_port):
        ...
        if src not in self.network:
            ...
            self.network.add_edge(dpid,src,weight=100)
            self.network.add_edge(src,dpid,weight=100)
